function align_info = VidIntanAlign(Videolist,protocol_path_list,pupil_dia,vid_path_list,FrameRate_defined,parStartEnd,plot_button)%,align_info)
...
% %         end
        ...
        load(protocol_path_list{1}); % get sample rate ...
        Time2Delete = trigger.trigTime_off;% for sessions with short distance infrared-trigger - first fv_on
        
        align_info(vid).intan_Time2Delete = Time2Delete;  
        align_info(vid).video_start = [VOI_Info.FrameBegin];
