function  process_protocol__reappraisal(protocollist,rhdlist,outputdir,rootdir) % [events, info, licks] =
...
for ii=1:size(pairs,2)
    ...
    trigData=dchannels(:,4); % trigger scanner LW
    ...
    if length(trigTime_on)>1
        display(['more than one laser triger in ' protocolfile '.mat - please check the file!']);
    end
    ...
    info.laser_on=trigTime_on;
    info.laser_off=trigTime_off;
    info.laser_dur=trigDur;
