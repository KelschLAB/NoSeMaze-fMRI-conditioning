function  process_protocol__reappraisal_ephys_2022__pupil(protocollist,rhdlist,outputdir,rootdir) % [events, info, licks] =
...
for ii=1:size(pairs,2)
    ...
    % fv_dur (here: 2.4 s)
    ToDelete= find( fv_dur > 1.05 | fv_dur < 0.95); %%% CAVE: CHANGE BACK AFTER FIRST SESSIONS
...
%     end
    ...
    if length(trigTime_on)==2 
        ...
        trigger.trigTimeEnd_on=trigTime_on(1);
        trigger.trigTimeEnd_off=trigTime_off(1);
