%% master_process_pupil_reappraisal_fMRI_2022.m
...
protocol_mainDir = '/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/04-pupil/01-reappraisal/01-processed_protocol_files';

% Load video pathlist and protocol pathlist
load(fullfile(vid_mainDir,'vid_pathlist_reappraisal.mat'),'vid_path_list');
load(fullfile(protocol_mainDir,'protocol_pathlist_reappraisal.mat'),'protocol_path_list');
...
if 1==0
    ...
    [pupil_dia,lid_dia] = pupil_load_and_fit_ellipse_fMRI_2022(Videolist_csv, Videolist, likelihood_threshold_pupil, likelihood_threshold_lid, vid_mainDir, plot_button);
...
if 1==1
    for vid=1:length(Videolist)
        ...
        Time2Delete=trigger.trigTime_off;
        ...
        LidDiameter = lid_dia(vid).d_mean_rot;
        ...
        for trial = 1:numel(events)
            ...
            OdorFrameCur = OdorFrames(trial).Loc_OdorFrames;
            FrameBaseStart = OdorFrameCur - (baseshift*VideoFrameRate_defined) + (0.7*VideoFrameRate_defined);
...
if length(Videolist_csv) < length(protocol_path_list)
    ...
    for pid=1:length(protocol_path_list)
        idx_animal = strfind(protocol_path_list{pid},'animal_');
        animal_name = protocol_path_list{pid}(idx_animal:(idx_animal+length('animal_')+1));
