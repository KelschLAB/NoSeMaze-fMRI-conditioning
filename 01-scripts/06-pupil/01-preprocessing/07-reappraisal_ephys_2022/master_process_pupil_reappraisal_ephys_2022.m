%% master_process_pupil_reappraisal_ephys_2022.m
...
vid_mainDir = '/zi-flstorage/data/jonathan/ICON_Autonomouse/02-raw-data/04-pupil/04_reappraisal_ephys_2022/03-videos_pupil';
protocol_mainDir = '/zi-flstorage/data/jonathan/ICON_Autonomouse/03-processed-data/04-pupil/04_reappraisal_ephys_2022/01-processed_protocol_files';

% Load video pathlist and protocol pathlist
load(fullfile(vid_mainDir,'vid_pathlist_reappraisal_ephys_2022.mat'),'vid_path_list');
load(fullfile(protocol_mainDir,'protocol_pathlist_reappraisal_ephys_2022.mat'),'protocol_path_list');
...
if 1==1
    ...
    lid_dia_v2=[];
    parpool(30)
    parfor vid = 1:numel(Videolist_csv)
        [pupil_dia(vid),lid_dia(vid),lid_dia_v2(vid)] = pupil_load_and_fit_ellipse_ephys_2022(Videolist_csv{vid}, Videolist{vid}, likelihood_threshold_pupil, likelihood_threshold_lid, vid_mainDir, plot_button);
    end
    save(fullfile(vid_mainDir,['pupil_dia.mat']),'pupil_dia','lid_dia','lid_dia_v2');
end
load(fullfile(vid_mainDir,'pupil_dia.mat'),'pupil_dia','lid_dia','lid_dia_v2');
...
if 1==0
    for ix=1:length(pupil_dia)
        ...
        idx=find(lid_dia(ix).d_mean < (nanmean(lid_dia(ix).d_mean)-3.*nanstd(lid_dia(ix).d_mean)) | lid_dia(ix).d_mean > (nanmean(lid_dia(ix).d_mean)+3.*nanstd(lid_dia(ix).d_mean)));
        [fdir,~,~]=fileparts(protocol_path_list{ix});[~,fname,~]=fileparts(fdir);
        disp(['For ' fname ' ' num2str(length(idx)) ' timepoints extended the range of mean +/- 3*STD and are set to NaN']);
        lid_dia(ix).d_mean(idx)=NaN;
    end
...
if 1==0
    ...
    lid_dia_scrubbed = pupil_scrubbing(Videolist_csv, lid_dia, vid_mainDir,plotter);
    save(fullfile(vid_mainDir,'pupil_dia_scrubbed.mat'),'pupil_dia_scrubbed','lid_dia_scrubbed','pupil_dia','lid_dia');
end
% load(fullfile(vid_mainDir,'pupil_dia_scrubbed.mat'),'pupil_dia_scrubbed','lid_dia_scrubbed','pupil_dia','lid_dia');
...
if 1==1
    for vid=1:length(Videolist)
        ...
        clear fv_on fv_off fv_on_align fv_off_align
