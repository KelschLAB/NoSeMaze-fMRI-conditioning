function  process_protocol__postVarenicline_ephys_2022__pupil(protocollist,rhdlist,outputdir,rootdir) % [events, info, licks] =
...
for ii=1:size(pairs,2)
    ...
    MaxTrialNum = length(session.trialmatrix)/2;
    ...
    % read laser trace of dchannel 1
    lasertrace=dchannels(:,1);
    
    % detect laser_on and laser_off
    laser_on=find(diff(lasertrace)==1)/freq;
    laser_off=find(diff(lasertrace)==-1)/freq;
    
    % calculate laser_dur (duration)
    laser_dur=laser_off-laser_on;
    
%     % in case you start matlab protocol twice in one intan recording
%     if length(find(laser_dur > 9)) > 1
%         limits=find(laser_dur>9);
%         laser_on([limits(1):1:limits(2)])= [];
%         laser_off([limits(1):1:limits(2)])= [];
%         laser_dur([limits(1):1:limits(2)])= [];
%     end
    
    % remove false signals detected (e.g. at the end or at the beginning) by controlling the
    % laser_dur (here: 2.4 s)
    ToDelete= find( laser_dur > 0.0055 | laser_dur < 0.0045); %%% CAVE: CHANGE BACK AFTER FIRST SESSIONS
    
    if ~isempty(ToDelete)
        % Case 1: False signals at the end
        if ToDelete(1) > length(session.trialmatrix) && ToDelete(end) > length(session.trialmatrix)
            laser_on(ToDelete(1):ToDelete(end))= [];
            laser_off(ToDelete(1):ToDelete(end))= [];
            laser_dur(ToDelete(1):ToDelete(end))= [];
            % Case 2: False signals at the start
        elseif ToDelete(1) < 20 && ToDelete(end) < 20
            laser_on(1:ToDelete(end))= [];
            laser_off(1:ToDelete(end))= [];
            laser_dur(1:ToDelete(end))= [];
            % Case 3: False signals in the middle, but very short
        elseif length(ToDelete) == 1 && ToDelete(1) > 1 && ToDelete(end) < length(session.trialmatrix) && laser_dur(ToDelete(1)) < 0.001
            laser_on(ToDelete(1))= [];
            laser_off(ToDelete(1))= [];
            laser_dur(ToDelete(1))= [];
        else
            error(['Check false laser_onsets manually (~=MaxTrialNum) in ' protocolfile '.mat'])
        end
    end
    
    % CONTROL: detected number of laser onsets equal to trials passed?
    MaxTrialNum = length(session.trialmatrix)/2;
    if length(laser_on) ~= MaxTrialNum
        error(['incorrect number of laser_onsets (~=MaxTrialNum) in ' protocolfile '.mat'])
    end
    
    % CONTROL: same number of laser ons and offs...
    if length(laser_on)~=length(laser_off)
        error(['unequal number of laser_on and laser_off in ' protocolfile '.mat'])
    end
    ...
    %% ODOR trials
    % TRIAL 1 - 160, 
    % CAVE: alternating with laser trials, but always starting session with odor
    for i=1:length(fv_on)
        
        % ODOR
        events((i-1)*2+1).fv_on=fv_on(i);
        events((i-1)*2+1).fv_off=fv_off(i);
        events((i-1)*2+1).fv_dur=fv_dur(i);
    ...
    for i=1:length(laser_on)
        ...
        % ODOR
        events((i-1)*2+2).laser_on=laser_on(i);
        events((i-1)*2+2).laser_off=laser_off(i);
        events((i-1)*2+2).laser_dur=laser_dur(i);
    ...
    k=strfind(intanfile,'_22'); % CC (in order to get the proper date
