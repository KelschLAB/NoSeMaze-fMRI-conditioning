%% master_plot_PupilAndLid_sD_sH_ephys_2022_pooled_data.m
...
load('/zi-flstorage/data/Jonathan/ICON_Autonomouse/02-raw-data/04-pupil/07-social_defeat_ephys_2022/03-videos_pupil/pupil_summary_all.mat');
...
if 1==1
    ...
    for jx=1:length(odor_list)
        ...
        for ix=1:length(summary_all_rp)
            ...
            data_rp{jx}=[data_rp{jx};summary_all_rp(ix).PupilBaseDiameterMatrix_Corrected([summary_all_rp(ix).odor_num]==odor_list(jx).code,:)];
...
%     end
    ...
    % print
    print('-dpsc',fullfile(outputDir,['Plots_PupilAndLid_socialDefeat_ephys_2022']),'-painters','-r400','-append');
    print('-dpdf',fullfile(outputDir,['Plots_PUPIL_INTRATRIAL_PupilAndLid_socialDefeat_ephys_2022']),'-painters','-r400');
...
if 1==1
    ...
    for jx=1:length(odor_list)
        ...
        for ix=1:length(summary_all_rp)
            %% Data for each range (=blocks)
            data_rp{jx}=[data_rp{jx};summary_all_rp(ix).LidBaseDiameterMatrix_Corrected([summary_all_rp(ix).odor_num]==odor_list(jx).code,:)];
%            %% Data for the whole session
%             if jx==1
%                 clear transposed_data
%                 transposed_data = summary_all_rp(ix).PupilDiameterMatrix';
...
%             end
    ...
    for jx=1:length(odor_list)
        data_rp{jx+length(odor_list)}=[];
        % Loop over animals (CAVE: only every third sessions in this
        % struct is a social_defeat session)
        for ix=1:length(summary_all_rp)
            %% Data for each range (=blocks)
            data_rp{jx+length(odor_list)}=[data_rp{jx+length(odor_list)};summary_all_rp(ix).LidBaseDiameterMatrix_Corrected([summary_all_rp(ix).odor_num]==odor_list(jx).code,:)];
            %% Data for the whole session
            if jx==1
                clear transposed_data
                transposed_data = summary_all_rp(ix).PupilDiameterMatrix';
    ...
    % print
    print('-dpsc',fullfile(outputDir,['Plots_PupilAndLid_social_defeat_ephys_2022']),'-painters','-r400','-append');
    print('-dpdf',fullfile(outputDir,['Plots_LID_INTRATRIAL_PupilAndLid_social_defeat_ephys_2022']),'-painters','-r400');
