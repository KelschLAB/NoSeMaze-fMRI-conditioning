%% master_plot_PupilAndLid_social_defeat_fMRI_2022_pooled_data.m
...
load('/home/jonathan.reinwald/ICON_Autonomouse/02-raw-data/04-pupil/02-social_defeat/03-videos_pupil/pupil_summary_all.mat');
...
%             end
    ...
    % legend
    ll=legend([sd{1}.mainLine,sd{2}.mainLine,sd{3}.mainLine],[odor_list_SD(1).name],[odor_list_SD(2).name],[odor_list_SD(3).name],'Location','SouthEast');   
    ...
    print('-dpdf',fullfile(outputDir,['Plots_PUPIL_INTRATRIAL_PupilAndLid_social_defeat_fMRI_2022']),'-painters','-r400');
...
if 1==0
    ...
    for jx=1:length(range)
        ...
        for ix=1:length(summary_all_rp_SD)
            %% Data for each range (=blocks)
            data_rp{jx}=[data_rp{jx};summary_all_rp_SD(ix).LidBaseDiameterMatrix_Corrected(range{jx},:)];
            %% Data for the whole session
            if jx==1
                clear transposed_data
                transposed_data = summary_all_rp_SD(ix).LidDiameterMatrix';
                data_long_rp(ix,:)=transposed_data(:)';    
                data_long_rp_smoothed(ix,:)=smooth(transposed_data(:)',smoothing_kernel);
            end
        end
