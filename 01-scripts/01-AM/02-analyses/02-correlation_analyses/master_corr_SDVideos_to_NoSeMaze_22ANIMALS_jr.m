%% master_corr_SDVideos_to_NoSeMaze_22ANIMALS_jr.m
...
[rho_mat,p_mat]=corr(myMat,myMat,'type','Pearson');
...
exportgraphics(fig(1),fullfile(outputDir,['correlation_matrix_SDVideosToNoSeMaze_22animals.png']),'ContentType','vector','BackgroundColor','none','Resolution',300);
% print('-dpsc',fullfile(outputDir,['correlation_matrix_SDVideosToNoSeMaze_22animals']),'-painters','-r400','-bestfit');
close all;


%% II. Individual plots
selection_xAxis = myVariableNames(contains(myVariableNames,'Rank') | contains(myVariableNames,'DS') | contains(myVariableNames,'Zscored'));
...
selection_yAxis{5} = myVariableNames(contains(myVariableNames,'FtoB'));
selection_yAxis{6} = myVariableNames(contains(myVariableNames,'PCA'));
...
%             axis square;
            ...
            tx1=text(ax.XLim(1)+diff(ax.XLim)*0.3,ax.YLim(1)+diff(ax.YLim)*0.1,['p_s_p=' num2str(round(p,2))]);
            tx2=text(ax.XLim(1)+diff(ax.XLim)*0.3,ax.YLim(1)+diff(ax.YLim)*0.2,['r_s_p=' num2str(round(r,2))]);  
        end
        ...
        exportgraphics(fig(2),fullfile(outputDir,['scatters_' selection_xAxis{ix} 'To' selection_yAxis{iy}{1}(1:end-2) '_22animals.png']),'ContentType','vector','BackgroundColor','none','Resolution',300);
