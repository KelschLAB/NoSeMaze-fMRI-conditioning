...
for an = 1:size(T,1)
     ...
     % import hierarchy from integrated Tube-Test (ITT)
    load(fullfile(data_dir{T.Autonomouse(an)}, 'full_hierarchy.mat'))
...
f.Units = 'centimeters';
f.Position = [3 3 4 4];
exportgraphics(f,fullfile(save_dir,'tube_DS_correlation_within_week_1_to_2.pdf'));
...
f.Units = 'centimeters';
f.Position = [3 3 4 4];
exportgraphics(f,fullfile(save_dir,'tube_rank_correlation_within_week_1_to_2.pdf'));



writetable(T,fullfile(save_dir,'tube_stability_source.xlsx'));