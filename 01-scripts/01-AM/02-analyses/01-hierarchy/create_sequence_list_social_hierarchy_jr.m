...
cd(workingDir);
pattern = fullfile(workingDir,'full_hierarchy*.mat');
...
for i = 1:length(files)
        ...
        if ~isempty(indices)
            ...
            for idx = 1:length(indices)
                ...
                if ~isempty(full_hierarchy(current_day).match_info{indices(idx)}) && isstruct(full_hierarchy(current_day).match_info{indices(idx)})
                    ...
                    winner_exit_time = [full_hierarchy(current_day).match_info{indices(idx)}.winner_exit_time];
                    winner_exit_times_currentday = [winner_exit_times_currentday; winner_exit_time'];
                    
                    winner_ID = {full_hierarchy(current_day).match_info{indices(idx)}.winner_ID}';
                    winner_IDs_currentday = vertcat(winner_IDs_currentday, winner_ID);
                    
                    loser_ID = {full_hierarchy(current_day).match_info{indices(idx)}.loser_ID}';
    ...
    writetable(myTable,fullfile(files(i).folder,[filename,'_days' num2str(day_range(1)) 'to' num2str(day_range(end)) '_sequence.csv']));
end