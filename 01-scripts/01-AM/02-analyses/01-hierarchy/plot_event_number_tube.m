...
f = figure;
histogram(collect_chasing,'Normalization','probability','FaceColor',[.5 .5 .5]);
ylabel('probability','FontSize',6);
xlabel({'number of detected','events per day'},'FontSize',6);
title('tube test','FontSize',8);
set(gca, 'FontSize', 6);
box('off')
f.Units = 'centimeters';
f.Position = [3 3 4 4];

exportgraphics(gcf, fullfile(save_dir,'tube_number_of_events_both_AM_combined.pdf'),'ContentType','vector','BackgroundColor','none');

% export source data
writetable(array2table(collect_chasing),fullfile(save_dir,'tube_number_of_events_both_AM_combined_source.xlsx'));
