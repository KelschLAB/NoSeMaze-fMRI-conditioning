function full_hierarchy = extract_chasing_from_full_hierarchy_jr(full_hierarchy,ops)
...
if ~isfield(ops,'threshold_lag_at_detector')
    ops.threshold_lag_at_detector = 1; % in seconds
end
...
for tt = 1:2
    ...
    tmp1 = strfind(abs(diff(tube_filtered.Detector))',[0 1 0]);% abs added by JR 14.12.2023
...
for day = 1:numel(unique_days)
   ...
   for ii = 1:numel(cur_chasings)
       ...
       full_hierarchy(day).match_info_chasing{winner_id,loser_id}(num).winner_entry_time = filtered_detections.Time_sec(cur_chasings(ii));
       full_hierarchy(day).match_info_chasing{winner_id,loser_id}(num).loser_entry_time = filtered_detections.Time_sec(cur_chasings(ii)+1);
       full_hierarchy(day).match_info_chasing{winner_id,loser_id}(num).winner_exit_time = filtered_detections.Time_sec(cur_chasings(ii)+2);
       full_hierarchy(day).match_info_chasing{winner_id,loser_id}(num).loser_exit_time = filtered_detections.Time_sec(cur_chasings(ii)+3);
       ...
       full_hierarchy(day).match_info_chasing{winner_id,loser_id}(num).winner_entry_Detector = filtered_detections.Detector(cur_chasings(ii));
       full_hierarchy(day).match_info_chasing{winner_id,loser_id}(num).winner_exit_Detector = filtered_detections.Detector(cur_chasings(ii)+2);
       full_hierarchy(day).match_info_chasing{winner_id,loser_id}(num).loser_entry_Detector = filtered_detections.Detector(cur_chasings(ii)+3);
       full_hierarchy(day).match_info_chasing{winner_id,loser_id}(num).loser_exit_Detector = filtered_detections.Detector(cur_chasings(ii)+4);
   end
