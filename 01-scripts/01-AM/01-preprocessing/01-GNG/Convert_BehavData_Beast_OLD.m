function [events] = Convert_BehavData_Beast_OLD(excelpath, savedir)
...
%% 
    if nargin < 2 % in case there is no input ... 
      ...
      excelpath = [pathname filename];
       % define savedir ... 
       savedir = 'C:\Users\sarah.ghanayem\Dropbox\MatLabData\Data\ICON\AM2\GNG-Task\Matlab_Output'; 
    end 
        ...
        for trial_cur = 1:TrialNum_cur;
            ...
            %it in 'events' variable (structure) ... 
               ...
               events(trial_cur).drop_or_not='No Drop';
                ...
                if events(trial_cur).licks_after_odor_aligned==0
                    events(trial_cur).drop_or_not='No Drop';
                end
...
%         filename_csv_short = filename(1:end-4); 
        savestring_cur = [savedir filesep IDcur '.mat']; 
