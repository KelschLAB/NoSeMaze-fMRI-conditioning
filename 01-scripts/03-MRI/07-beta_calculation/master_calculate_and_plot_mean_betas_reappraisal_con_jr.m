%% master_calculate_and_plot_mean_betas_reappraisal_con_jr.m
% Reinwald, Jonathan
% last update: 02/2023
...
GLMdir = '/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/07-reappraisal_control_2023/05-GLM/03-results/'
    ...
    if ~isempty(masklist)
        ...
        for mx = 1:length(masklist)
            ...
            if nansum(nansum(nansum(img_mask)))>0
                ...
                % change structure to fit to normal task
                for ix=1:length(contrast_info); contrast_info2.names{ix} = contrast_info{ix}.names; contrast_info2.test{ix} = contrast_info{ix}.test; end;
                contrast_info = contrast_info2;
                ...
                %% 2. From betas.nii
                % Load SPM.mat
                load(fullfile(GLM1stlevel_dir,'ZI_M230906A','SPM.mat'));
