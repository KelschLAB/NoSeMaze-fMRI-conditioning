...
for lx=3%:3;
    for kx=1%:9;
        ...
        d_BOLD=zeros(80,8,83);
        nd_BOLD=zeros(80,8,83);
        for ix=1:83;
            for jx=1:8;
                if length(animal(ix).(preproc_sel{lx}).(region_sel{kx}).BOLD.(['timebin_' num2str(jx)]).tc(animal(ix).SigmaB.Rew_code.val==1))==79;
                    dh_BOLD(2:80,jx,ix)=(animal(ix).(preproc_sel{lx}).(region_sel{kx}).BOLD.(['timebin_' num2str(jx)]).tc(animal(ix).SigmaB.Rew_code.val==1));
                elseif length(animal(ix).(preproc_sel{lx}).(region_sel{kx}).BOLD.(['timebin_' num2str(jx)]).tc(animal(ix).SigmaB.Rew_code.val==1))==80;
                    dh_BOLD(1:80,jx,ix)=(animal(ix).(preproc_sel{lx}).(region_sel{kx}).BOLD.(['timebin_' num2str(jx)]).tc(animal(ix).SigmaB.Rew_code.val==1));
                end;
                if length(animal(ix).(preproc_sel{lx}).(region_sel{kx}).BOLD.(['timebin_' num2str(jx)]).tc(animal(ix).SigmaB.Rew_code.val==0))==79;
                    nd_BOLD(2:80,jx,ix)=(animal(ix).(preproc_sel{lx}).(region_sel{kx}).BOLD.(['timebin_' num2str(jx)]).tc(animal(ix).SigmaB.Rew_code.val==0));
                elseif length(animal(ix).(preproc_sel{lx}).(region_sel{kx}).BOLD.(['timebin_' num2str(jx)]).tc(animal(ix).SigmaB.Rew_code.val==0))==80;
                    nd_BOLD(1:80,jx,ix)=(animal(ix).(preproc_sel{lx}).(region_sel{kx}).BOLD.(['timebin_' num2str(jx)]).tc(animal(ix).SigmaB.Rew_code.val==0));
                end
            end
        end
