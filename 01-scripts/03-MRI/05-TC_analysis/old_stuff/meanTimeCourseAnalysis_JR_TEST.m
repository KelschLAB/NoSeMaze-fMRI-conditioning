...
%         Pcur=spm_select('FPlist',fdir,['^s_rwst_a1_u_del5_' fname '_c1_c2t_icaden25_16-Feb-2020.nii$']);
    ...
    ll.LineWidth=2;
    ll.Color=[0 0.5 0.5];
    text(3.8,-0.3,'Reward','Color',[0 0.5 0.5],'FontSize',14);
    ...
    [val,ind]=max(mean(coef_expect));
    for kx=1:size(coef_expect,2);
        [h_tt(kx),p_tt(kx),ci_tt{kx},stats_tt{kx}]=ttest2(coef_expect(:,kx),coef_expect(:,ind));
    end;
    
    h=notBoxPlot(coef_expect);
    for sz=1:8;
        h(sz).data.MarkerSize=2;
    end;
    hold on;
    plot(nanmean(coef_expect,1),'Color',[0 0 0],'LineWidth',2);
    ax.XLabel.String='[s], time bin: TR';
    ax.XLim=[1.5 8.5];
    ax.XTick=[0.5:1:8.5];
    ax.XTickLabel=[-1.2:1.2:8.4];
