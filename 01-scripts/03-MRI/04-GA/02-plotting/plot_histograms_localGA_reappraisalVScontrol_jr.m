%% plot_histograms_localGA_reappraisalVScontrol_jr.m
...
block_selection{3}='TPnoPuff11to40';
block_selection{4}='TPnoPuff81to120';
block_selection{5}='TPnoPuff11to40';
block_selection{6}='TPnoPuff81to120';

% graph metrics
selected_metrics = {'strength','degree','bci','cc','PI'};
...
% color
histogram_color={[204/255 51/255 204/255];[0 160/255 227/255];[204/255 51/255 204/255];[0 160/255 227/255];[0/255 83/255 83/255];[122/255 46/255 84/255]};%[1,0,0];[0,1,0]}%
% binwidth
...
if separated_hemisphere==2
    inputDir_GA_task = ['/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/01-reappraisal/07-GA/01-gstruct_files/' cormat_version_task filesep 'separated_v2_2023_hemisphere' filesep binarization_method '_' connectedness];
    inputDir_GA_control = ['/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/07-reappraisal_control_2023/07-GA/01-gstruct_files/' cormat_version_control filesep 'separated_v2_2023_hemisphere' filesep binarization_method '_' connectedness];
...
for mx = 1:length(minthr_ind_range)
    ...
    %% Output directory
    if separated_hemisphere==2
        outputDir = ['/home/jonathan.reinwald/ICON_Autonomouse/04-outputs/03-MRI/01-reappraisal/06-GA/' cormat_version_task filesep 'separated_v2_2023_hemisphere' filesep binarization_method '_' connectedness '/histograms_localMetrics_taskVScontrol'];
    elseif separated_hemisphere==1
        outputDir = ['/home/jonathan.reinwald/ICON_Autonomouse/04-outputs/03-MRI/01-reappraisal/06-GA/' cormat_version_task filesep 'separated_hemisphere' filesep binarization_method '_' connectedness '/histograms_localMetrics_taskVScontrol'];
    elseif separated_hemisphere==0
        outputDir = ['/home/jonathan.reinwald/ICON_Autonomouse/04-outputs/03-MRI/01-reappraisal/06-GA/' cormat_version_task filesep 'combined_hemisphere' filesep binarization_method '_' connectedness '/histograms_localMetrics_taskVScontrol'];
    end
    ...
    for curr_metricID = [1,4]%1:length(selected_metrics)
        % set figure
        ...
        for bl=1:length(block_selection)
            % load gstruc-file
            if bl==1 | bl==2
                load(fullfile(inputDir_GA_task,['gstruc_' block_selection{bl} '_p.mat']));
            elseif bl==3 | bl==4
                load(fullfile(inputDir_GA_control,['gstruc_' block_selection{bl} '_p.mat']));                
            elseif bl==5 
                load(fullfile(inputDir_GA_task,['gstruc_' block_selection{bl} '_p.mat']));      
                gstruc1 = gstruc;
                load(fullfile(inputDir_GA_task,['gstruc_' block_selection{bl+1} '_p.mat']));
                gstruc3 = gstruc;
            elseif bl==6
                load(fullfile(inputDir_GA_control,['gstruc_' block_selection{bl-1} '_p.mat']));      
                gstruc1 = gstruc;
                load(fullfile(inputDir_GA_control,['gstruc_' block_selection{bl} '_p.mat']));
                gstruc3 = gstruc;
            end
            ...
            if any(strcmp(myFieldnames,['l_' selected_metrics{curr_metricID}]))
                ...
                elseif bl==5 || bl==6
                    myMetrics.(myFieldnames{strcmp(myFieldnames,['l_' selected_metrics{curr_metricID}])}).(block_selection{bl}).values=[gstruc3([minthr_ind:maxthr_ind],:).(myFieldnames{strcmp(myFieldnames,['l_' selected_metrics{curr_metricID}])})]-[gstruc1([minthr_ind:maxthr_ind],:).(myFieldnames{strcmp(myFieldnames,['l_' selected_metrics{curr_metricID}])})];                    
                end
            else
            ...
            if any(strcmp(myFieldnames,['l_' selected_metrics{curr_metricID}]))
                ...
                hh2(bl).EdgeColor=histogram_color{bl};
                hh2(bl).LineWidth=2.5;
                hold on;
                
                % save source data as csv-file
                SourceData = array2table(myMetrics.(myFieldnames{strcmp(myFieldnames,['l_' selected_metrics{curr_metricID}])}).(block_selection{bl}).values(:));
                if bl<3
                    writetable(SourceData,fullfile(outputDir,['SourceData_Histogram_' selected_metrics{curr_metricID} '_' block_selection{bl} '_TaskCohort.csv']),'WriteVariableNames',true,'WriteRowNames',true)
                elseif bl<5
                    writetable(SourceData,fullfile(outputDir,['SourceData_Histogram_' selected_metrics{curr_metricID} '_' block_selection{bl} '_ControlCohort.csv']),'WriteVariableNames',true,'WriteRowNames',true)
                elseif bl==5
                    writetable(SourceData,fullfile(outputDir,['SourceData_Histogram_' selected_metrics{curr_metricID} '_DiffBlock3vs1_TaskCohort.csv']),'WriteVariableNames',true,'WriteRowNames',true)
                else
                    writetable(SourceData,fullfile(outputDir,['SourceData_Histogram_' selected_metrics{curr_metricID} '_DiffBlock3vs1_ControlCohort.csv']),'WriteVariableNames',true,'WriteRowNames',true)
                end
...
%                     [p_value,~,stats]=signrank(hh(bl-1).Data,hh(bl).Data);
                    [p_value, observeddifference, effectsize] = permutationTest(hh(bl-1).Data, hh(bl).Data, 10000)
                    tx = text(ax(bl).XLim(2)*.6,ax(bl).YLim(2)*.8,['p=' num2str(round(p_value,5))]);
...
%         end
        ...
        if ishandle(fig(curr_metricID))
            ...
            exportgraphics(fig(curr_metricID),fullfile(outputDir,['Histogram_' selected_metrics{curr_metricID} '.png']),'Resolution',300);
%             print('-dpsc',fullfile(outputDir,['Histograms']),'-painters','-r400','-bestfit','-append');
        end
    end
