...
if separated_hemisphere==1
    workdir = ['/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/06-FC/01-BASCO/' cormat_version '/beta4D' filesep 'separated_hemisphere'];
elseif separated_hemisphere==0
    workdir = ['/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/06-FC/01-BASCO/' cormat_version '/beta4D' filesep 'combined_hemisphere'];
end
% Output directory
if separated_hemisphere==1
    outputdir = ['/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/07-GA/01-gstruct_files/' cormat_version filesep 'separated_hemisphere' filesep binarization_method '_' connectedness_val];
elseif separated_hemisphere==0
    outputdir = ['/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/07-GA/01-gstruct_files/' cormat_version filesep 'combined_hemisphere' filesep binarization_method '_' connectedness_val];
end
mkdir(outputdir);
...
if 1==1
    for ix = [14]%[2,7,12,17]%[2,6,9,11,15]%1:size(cormat_files,1)
        clear gstruc cormat
        % Load Cormat
...
% % %         end      
        ...
        if 1==1
            ...
            % Save gstruc
            save([outputdir filesep 'auc_struc_' curr_name '_p.mat'],'auc_struc');
        end      
    end
end
