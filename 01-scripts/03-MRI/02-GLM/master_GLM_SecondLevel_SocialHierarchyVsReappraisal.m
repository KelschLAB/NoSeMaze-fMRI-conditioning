...
% as we use social defeat and social hierarchy data in here, we have *_sh
% and *_rp folders
...
GLM_name = ['HRF' HRF_TCbased '_' HRF_infopath '_EPI_' epiPrefix(1:end) '___ROI_' regressorsSuffix_sh(2:end-4) '___COV_' covarSuffix_sh(2:3) '___ORTH_' num2str(orth) '___20-Jan-2023'];%'___20-Jan-2023'
...
selection_sh = ismember(mySubjects_sh,mySubjects_rp);
selection_rp = ismember(mySubjects_rp,mySubjects_sh);
subjects = [1:sum(selection_rp)];
Pfunc_social_hierarchy=Pfunc_social_hierarchy(selection_sh);
...
if 1==1
    ...
    for i_SH = 1:length(contrast_names_SH) %[30:32]% (for selection of the regressors v6 in the defeat task including blocks 1-3)
        ...
        for i_RP = 1:length(contrast_names_RP) %[4,8]% (for selection of the regressors v3 in the hierarchy task including blocks 1-3)
            ...
            outputDir = fullfile(resultsDir,['HRF' HRF_TCbased '_' HRF_infopath '_EPI_' epiPrefix(1:end) '____SH_ROI_' regressorsSuffix_sh(2:end-4) '_vs_RP_ROI_' regressorsSuffix_rp(2:end-4)],[contrast_names_SH{i_SH} 'vs' contrast_names_RP{i_RP}]);
            if ~exist(outputDir)
                mkdir(outputDir);
            end
            ...
            for subj = 1:length(subjects)
                ...
                % Fill in Scans
                matlabbatch{1}.spm.stats.factorial_design.des.pt.pair(subj).scans = {scan_SH;scan_RP};
            end
...
if 1==0
    ...
    % definition of output directory
    outputDir = fullfile(resultsDir,GLM_name,[contrast_names_RP{i_SH} 'vs' contrast_names_RP{i_RP}]);
    if ~exist(outputDir)
        mkdir(outputDir);
    end
    ...
    for subj = 1:length(subjects)
        ...
        % Define Scans
        if contrast_IDs_SH(i_SH)<10
            scan_SH = fullfile(firstlevelDir_RP,subjAbrev,['con_000' num2str(contrast_IDs_RP(i_SH)) '.nii']);
        else
            scan_SH = fullfile(firstlevelDir_RP,subjAbrev,['con_00' num2str(contrast_IDs_RP(i_SH)) '.nii']);
        end
