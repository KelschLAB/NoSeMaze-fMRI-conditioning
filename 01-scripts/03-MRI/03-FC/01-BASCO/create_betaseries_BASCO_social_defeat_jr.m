function create_betaseries_BASCO_social_defeat_jr(Pfunc_all,main_dir,vname);
...
% load anaobj
load([main_dir filesep 'output' filesep 'out_estimated_social_defeat_' vname '.mat']);
...
for subj = 1:length(Pfunc_all)
    ...
    for jx = 1:length(anaobj{1,1}.Ana{1,1}.AnaDef.Cond)
        ...
        %% CD1 familiar:
        if strcmp(anaobj{1,1}.Ana{1,1}.AnaDef.Cond{jx},'CD1 familiar')
            start_val = 0;
        elseif strcmp(anaobj{1,1}.Ana{1,1}.AnaDef.Cond{jx},'CD1 unknown')
            start_val = 30;
        elseif strcmp(anaobj{1,1}.Ana{1,1}.AnaDef.Cond{jx},'129 sv female')
            start_val = 60;
        end
        
        %
        counter = 1;
        for ix = [1:3]+start_val
            V = spm_vol(deblank(curr_filelist(ix,:)));
            V_all = V;
            [fdir, fname, ext]=fileparts(V.fname);
            V_all.fname = fullfile(beta4D_dir,[subjAbrev '_betaseries_' vname '_' file_name '_1to3.nii'])
            V_all.n(1)=counter;
            spm_write_vol(V_all,spm_read_vols(V));
            % Update counter
            counter = counter+1;
        ...
        %%
        counter = 1;
        for ix = [4:30]+start_val
            V = spm_vol(deblank(curr_filelist(ix,:)));
            V_all = V;
            [fdir, fname, ext]=fileparts(V.fname);
            V_all.fname = fullfile(beta4D_dir,[subjAbrev '_betaseries_' vname '_' file_name '_4to30.nii'])
            V_all.n(1)=counter;
            spm_write_vol(V_all,spm_read_vols(V));
            % Update counter
            counter = counter+1;
