...
% define paths...
protocol_dir = '/zi-flstorage/data/Jonathan/ICON_Autonomouse/03-processed-data/03-MRI/01-reappraisal/01-processed_protocol_files';
    ...
    for jx = 1:length(anaobj{1,1}.Ana{1,1}.AnaDef.Cond)
        ...
        if strcmp(anaobj{1,1}.Ana{1,1}.AnaDef.Cond{jx},'Lavender')
            ...
            for ix = 1:20
                ...
                V_all = V;
                [fdir, fname, ext]=fileparts(V.fname);
                V_all.fname = fullfile(beta4D_dir,[subjAbrev '_betaseries_' vname '_Odor1to20.nii'])
                V_all.n(1)=counter;
                spm_write_vol(V_all,spm_read_vols(V));
                % Update counter
                counter = counter+1;
            ...
            for ix = 21:40
                ...
                V_all = V;
                [fdir, fname, ext]=fileparts(V.fname);
                V_all.fname = fullfile(beta4D_dir,[subjAbrev '_betaseries_' vname '_Odor21to40.nii'])
                V_all.n(1)=counter;
                spm_write_vol(V_all,spm_read_vols(V));
                % Update counter
                counter = counter+1;
        ...
        elseif strcmp(anaobj{1,1}.Ana{1,1}.AnaDef.Cond{jx},'TP NoPuff')
            ...
            %%
            counter = 1;
            for ix = start_num:start_num+19
                V = spm_vol(deblank(curr_filelist(ix,:)));
                V_all = V;
                [fdir, fname, ext]=fileparts(V.fname);
                V_all.fname = fullfile(beta4D_dir,[subjAbrev '_betaseries_' vname '_TPnoPuff1to20.nii'])
                V_all.n(1)=counter;
                spm_write_vol(V_all,spm_read_vols(V));
                % Update counter
                counter = counter+1;
            ...
            %%
            counter = 1;
            for ix = start_num+20:start_num+39
                V = spm_vol(deblank(curr_filelist(ix,:)));
                V_all = V;
                [fdir, fname, ext]=fileparts(V.fname);
                V_all.fname = fullfile(beta4D_dir,[subjAbrev '_betaseries_' vname '_TPnoPuff21to40.nii'])
                V_all.n(1)=counter;
                spm_write_vol(V_all,spm_read_vols(V));
                % Update counter
                counter = counter+1;
