%% master_NBS_cormat_reappraisalVScontrol_jr.m
...
trial_selection{1}='TPnoPuff81to120'; trial_selection{2}='TPnoPuff11to40';
...
cormat_bl3_con = cormat;
load(fullfile(inputDir_control,[cormat_control '_' trial_selection{2} '.mat']))
cormat_bl1_con = cormat;

load(fullfile(inputDir_task,[cormat_task '_' trial_selection{1} '.mat']))
cormat_bl3_task = cormat;
load(fullfile(inputDir_task,[cormat_task '_' trial_selection{2} '.mat']))
cormat_bl1_task = cormat;

% calculate diff matrices
for ix=1:24; cormat_diff_con{1,ix}=cormat_bl3_con{1,ix}-cormat_bl1_con{1,ix}; end
for ix=1:24; cormat_diff_task{1,ix}=cormat_bl3_task{1,ix}-cormat_bl1_task{1,ix}; end
% concatenation of diff matrices
cormat_diff_all=[cormat_diff_con,cormat_diff_task];
% save diff matrices as 3D-vector
Mat=cat(3,cormat_diff_all{:});
% sorting
for i = 1:size(Mat,3)
    Mat(:,:,i)=Mat(sorting,sorting,i);
end
save(fullfile(outputDir,['Mat_' cormat_task '_' trial_selection{1} 'VS' trial_selection{2} '.mat']),'Mat');
...
csvwrite(fullfile(outputDir,['GLM_design_' cormat_task '_' trial_selection{1} 'VS' trial_selection{2} '.txt']),GLM_design);
...
% sorting
for i = 1:length(cormat_diff_con)
    cormat_diff_con{i}=cormat_diff_con{i}(sorting,sorting);
    cormat_diff_task{i}=cormat_diff_task{i}(sorting,sorting);
end

% t-statistic
[T p p2 fdrmat meanval stdab] = lei_ttest2(cormat_diff_con,cormat_diff_task, 0.05);
...
UI.matrices.ui=fullfile(outputDir,['Mat_' cormat_task '_' trial_selection{1} 'VS' trial_selection{2} '.mat']);
UI.design.ui=fullfile(outputDir,['GLM_design_' cormat_task '_' trial_selection{1} 'VS' trial_selection{2} '.txt']);
...
% source data
plot_data  = data;
SourceData = array2table(plot_data.*-1,'VariableNames',names,'RowNames',names);
writetable(SourceData,fullfile(outputDir,['SourceData_NBS_reappraisalVScontrol_' cormat_control 'VS' cormat_task '_thresh' num2str(thres) '_T_' num2str(thr_t) '.csv']),'WriteVariableNames',true,'WriteRowNames',true)
