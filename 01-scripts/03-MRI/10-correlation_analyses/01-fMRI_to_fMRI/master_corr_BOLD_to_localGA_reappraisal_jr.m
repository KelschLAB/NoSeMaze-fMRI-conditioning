%% master_corr_BOLD_to_localGA_reappraisal_jr.m
...
% load('/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/01-reappraisal/05-GLM/03-results/HRFlongTC_withoutOnset_from2sHRF-GLM_EPI_wave_10cons_med1000_msk_s6_wrst_a1_u_despiked_del5____ROI_v22___COV_v1___ORTH_1___DERDISP0___20-Jan-2023/secondlevel/Odor_NoPuff_Bl3_vs_Odor_NoPuff_Bl1_11to40/mask_activation_v22_OdorBl3vsBl1_T001.mat')
% load('/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/01-reappraisal/05-GLM/03-results/HRFlongTC_withoutOnset_from2sHRF-GLM_EPI_wave_10cons_med1000_msk_s6_wrst_a1_u_despiked_del5____ROI_v22___COV_v1___ORTH_1___DERDISP0___20-Jan-2023/secondlevel/Odor_NoPuff_Bl3_vs_Odor_NoPuff_Bl1_11to40/mask_deactivation_v22_OdorBl3vsBl1_T001.mat')
...
for ix=1:length(local_metrics)   
    ...
    for jx=1:length(names)        
        [h_p(ix,jx),p_p(ix,jx)]=corr(([res.mean_beta_TP_Puff_Bl3]'-[res.mean_beta_TP_Puff_Bl1_11to40]'),([res_auc_struc.(local_metrics{ix}).TPnoPuff81to120.(names{jx})]-[res_auc_struc.(local_metrics{ix}).TPnoPuff11to40.(names{jx})]),'type','Pearson');
...
exportgraphics(fig(1),fullfile(outputDir,['Correlation_BetaCoeffToLocalGA_overview.pdf']),'Resolution',300);
...
local_metrics_selected = [find(contains(local_metrics,'l_cc') | contains(local_metrics,'l_E_loc') | contains(local_metrics,'l_PI') | contains(local_metrics,'l_strength') | contains(local_metrics,'l_degree') |contains(local_metrics,'l_bci'))]';
...
for jx=[regions_selected]
    ...
    for ix=[local_metrics_selected]
        % figure
        fig(counter_fig+1)=figure('visible', 'off');
        set(gcf,'Units','Normalized','OuterPosition',[0,0.04,0.2,0.7]);
        % Statistics
        % 1. Pearson
        [h_p(counter_reg,counter_metrics),p_p(counter_reg,counter_metrics)]=corr(([res.mean_beta_TP_Puff_Bl3]'-[res.mean_beta_TP_Puff_Bl1_11to40]'),([res_auc_struc.(local_metrics{ix}).TPnoPuff81to120.(names{jx})]-[res_auc_struc.(local_metrics{ix}).TPnoPuff11to40.(names{jx})]),'type','Pearson');
        % 2. Spearman
        [h_sp(counter_reg,counter_metrics),p_sp(counter_reg,counter_metrics)]=corr(([res.mean_beta_TP_Puff_Bl3]'-[res.mean_beta_TP_Puff_Bl1_11to40]'),([res_auc_struc.(local_metrics{ix}).TPnoPuff81to120.(names{jx})]-[res_auc_struc.(local_metrics{ix}).TPnoPuff11to40.(names{jx})]),'type','Spearman');
        
        
        subplot(2,2,1:2);
        % scatter plot
        sc(1)=scatter(([res.mean_beta_TP_Puff_Bl3]'-[res.mean_beta_TP_Puff_Bl1_11to40]'),([res_auc_struc.(local_metrics{ix}).TPnoPuff81to120.(names{jx})]-[res_auc_struc.(local_metrics{ix}).TPnoPuff11to40.(names{jx})]));
        ...
        % scatter plot
        clear A1 A2 B1 B2
        [~,A1]=sort([res.mean_beta_TP_Puff_Bl3]'-[res.mean_beta_TP_Puff_Bl1_11to40]');
        [~,B1]=sort(A1);
        ...
        % print
        [annot, srcInfo] = docDataSrc(fig(counter_fig+1),outputDir,mfilename('fullpath'),logical(1))
        exportgraphics(fig(counter_fig+1),fullfile(outputDir,['Correlation_BetaCoeffToLocalGA_' local_metrics{ix} '_' names{jx} '.pdf']),'Resolution',300);
        print('-dpsc',fullfile(outputDir,['Correlation_BetaCoeffToLocalGA_individualMetrics']),'-painters','-r400','-append','-fillpage');
