%% master_corr_SHandSDVideos_to_GA_CD1fam_22ANIMALS_jr.m
...
for metric_idx = 1:length(myLocalMetrics)    
    for region_idx = 1:length(region_selection)
        myMetric = [auc_struc.(myLocalMetrics{metric_idx})]';
        T_GA = [T_GA,table(myMetric(selection,strcmp(names,region_selection{region_idx})),'VariableNames',{[myLocalMetrics{metric_idx} '_' region_selection{region_idx}]})];
    end
end
...
exportgraphics(fig(1),fullfile(outputDir,['correlation_matrix_SDVideosToNoSeMaze_22animals_' corrType '.pdf']),'ContentType','vector','BackgroundColor','none','Resolution',300);
exportgraphics(fig(1),fullfile(outputDir,['correlation_matrix_SDVideosToNoSeMaze_22animals_' corrType '.png']),'ContentType','vector','BackgroundColor','none','Resolution',300);
...
for ix = 1:length(selection_xAxis)
    ...
    for iy=1:length(selection_yAxis)
        ...
        exportgraphics(fig(2),fullfile(outputDir,['scatters_' selection_xAxis{ix} 'ToGA_' selection_yAxis{iy}{1}(strfind_(2)+1:end) '_' corrType '_22animals.pdf']),'ContentType','vector','BackgroundColor','none','Resolution',300);
        exportgraphics(fig(2),fullfile(outputDir,['scatters_' selection_xAxis{ix} 'ToGA_' selection_yAxis{iy}{1}(strfind_(2)+1:end) '_' corrType '_22animals.png']),'ContentType','vector','BackgroundColor','none','Resolution',300);
