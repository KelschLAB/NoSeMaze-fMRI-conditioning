%% master_calculate_DVARS_reappraisal_jr.m
...
% EPI selection
EPI_prefix = {'med1000_msk_s6_wrst_a1_u_del5_',...
    'med1000_msk_s6_wrst_a1_u_despiked_del5_',...
    'wave_10cons_med1000_msk_s6_wrst_a1_u_del5_',...
    'wave_30cons_med1000_msk_s6_wrst_a1_u_despiked_del5_',...
    'wave_10cons_med1000_msk_s6_wrst_a1_u_despiked_del5_',...
    'med1000_msk_s6_scrub_0_1_lin_wrst_a1_u_del5_',...
    'med1000_msk_s6_scrub_0_1_lin_wrst_a1_u_despiked_del5_',...
    'med1000_msk_s6_regfilt_motcsfder_wrst_a1_u_del5_',...
    'scrub_0_1_lin_wave_10cons_med1000_msk_s6_wrst_a1_u_despiked_del5_',...
...
% EPI names
EPI_name = {'noMotionCorrection','AFNI','WD10','WD30_AFNI','WD10_AFNI','Scrub01','Scrub01_AFNI','regfilt','Scrub01_WD_AFNI','DVARSscrub01_WD_AFNI','DVARSscrub01_WD'}
...
if 1==0
    ...
    for EPI_idx = 1:length(EPI_prefix)
        ...
        outputdir='/home/jonathan.reinwald/ICON_Autonomouse/04-outputs/03-MRI/01-reappraisal/01-preprocessing/01-motion/DVARS'
        ...
        for subj=1:size(Pfunc_reappraisal,2)
            ...
            if contains(EPI_prefix{EPI_idx},'wave')
                EPI=spm_select('ExtFPlist',[fpath filesep 'wavelet'],['^' EPI_prefix{EPI_idx} fname '_c1_c2t_wds.nii'],[1:2000]);
            else
                EPI=spm_select('ExtFPlist',fpath,['^' EPI_prefix{EPI_idx} fname '_c1_c2t.nii'],[1:2000]);
            end
            V_EPI=spm_vol(EPI);
            ...
            if contains(EPI_prefix{EPI_idx},'despiked')
                MovPar=[spm_load(spm_select('FPList',fpath,'^rp_despiked_del5_Z.*.txt'))];
            else
...
if 1==1
    ...
    for EPI_idx = 1:length(EPI_prefix)
        %% loop over subjects
        for subj=1:size(Pfunc_reappraisal,2)
            DVARS_info.(EPI_name{EPI_idx}).corrcoeffFDtoDVARS(subj,1) = corr(DVARS_info.(EPI_name{EPI_idx}).DVARS(subj,[1:end])',DVARS_info.(EPI_name{EPI_idx}).FD_SNiP(subj,[2:end])');
            
            % FD: definition and loading (either despiked or not)
            [fpath, fname, ext]=fileparts(Pfunc_reappraisal{subj});
            fpath = ['/zi-flstorage/data/jonathan' fpath(24:end)];
            if contains(EPI_prefix{EPI_idx},'despiked')
                MovPar=[spm_load(spm_select('FPList',fpath,'^regressors_despiked_detrmot_csf_der.txt'))];
            else
                MovPar=[spm_load(spm_select('FPList',fpath,'^regressors_detrmot_csf_der.txt'))];
            end
            MovPar(:,[1:3])=MovPar(:,[1:3])./10;
            %             for kx=1:15
            ...
            %                 DVARS_info.(EPI_name{EPI_idx}).partialcorrcoeffFDtoDVARS(subj,kx) = partialcorr(DVARS_info.(EPI_name{EPI_idx}).DVARS(subj,[(kx-1)*100+1:kx*100])',DVARS_info.(EPI_name{EPI_idx}).FD_SNiP(subj,[(kx-1)*100+2:kx*100+1])',MovPar([(kx-1)*100+2:kx*100+1],:));
            %             end
        end
        % fill matrices
