%% master_preprocessing_social_defeat_jr.m
...
rawdir='/home/jonathan.reinwald/ICON_Autonomouse/02-raw-data/03-MRI/02-social_defeat/01-fMRI_data';
procdir='/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/02-preprocessing';
cd(procdir)

% Predefine path of scanlist
scans='/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/03-filelists/scanlist_social_defeat_jr.csv';%path of scan list as a csv
...
if 1==0
    ...
    for jscan=1:size(subj_ID,1)                                                                           % strcmp -> contains; 171216; strcmp for EPIs due to EPI tests in the scan list
        episcan_social_defeat(jscan)=strcmp(deblank(char(examn(jscan))),'EPI_social_defeat');                                % strcmp goes through examination column, each cell with input 'EPI 1300' gets the logical 1% strcmp goes through examination column, each cell with input 'EPI 1300' gets the logical 1
    ...
    epiind=find(episcan_social_defeat);                                                                               % returns a vector containing the linear indices of each nonzero element in array episcan; lw
    for jepi=1:numel(epiind)
        ...
        for jscan=1:size(indc,1)
            if ~isempty(indc{jscan})
                Pfunc_social_defeat(jepi)=path_all(jscan);                                                            %Pfunc contains the pathes of the EPIs; lw
            end
        end
    end
    ...
    for jphy=1:length(Pfunc_social_defeat);
        [fpath,~,~]=fileparts(Pfunc_social_defeat{jphy})
        [fpath,fname,~]=fileparts(fpath)
        if length(fname)==1
            Pphysio_social_defeat(jphy,:)=['/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/04-physio/' num2str(fpath(end-1:end)) '-#0' num2str(fname) '.txt'];            
        elseif length(fname)==2
            Pphysio_social_defeat(jphy,:)=['/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/04-physio/' num2str(fpath(end-1:end)) '-#' num2str(fname) '.txt'];
        end
    end
    
    save('/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/03-filelists/filelist_ICON_social_defeat_jr.mat','P3d_1','Pdmap_1','Pdmap_2','P3d_2','Pfunc_social_defeat','Pphysio_social_defeat')
end
...
if 1==0
    for ix=1:size(Pfunc_social_defeat,2);
        [fpath fname ext]=fileparts(Pfunc_social_defeat{ix});
...
if 1==0
    for ix=1:size(P3d_2,2);
        [fdir, fname, ext]=fileparts(Pfunc_social_defeat{ix});
        Pfunc_cur=spm_select('ExtFPList',fdir,['^del5_' fname '.nii'],1);
        Pfunc_cur1=spm_select('ExtFPList',fdir,['^del5_' fname '.nii'],str2num(Pfunc_social_defeat{ix}(end-3:end))-5);
...
%% ------------------------- AFNI despiking ------------------------------%
% Despiking before unwarping and realignment
if 1==0
    for ix=1:length(Pfunc_social_defeat)
...
% % %         Pcur=deblank(Pfunc_social_defeat{ix});
% % %         [fdir, fname, ext]=fileparts(Pcur);
% % %         cd(fdir);
% % %         %         Pcur3=(['del5_' fname '.mat']);
% % %         %         syscmd=['rm ' Pcur3];
% % %         %         system(syscmd);
% % %         Pcur1=(['del5_' fname '.nii']);
% % %         Pcur2=(['orig_del5_' fname '.nii']);
...
if 1==0
    for ix=1:size(P3d_2,2);
        ...
        Pfunc_cur3=spm_select('ExtFPList',fdir,['^u_del5_' fname '.nii'],str2num(Pfunc_social_defeat{ix}(end-3:end))-10);
        Pfunc_cur4=spm_select('ExtFPList',fdir,['^del5_' fname '.nii'],1);
        Pfunc_cur5=spm_select('ExtFPList',fdir,['^' fname '.nii'],str2num(Pfunc_social_defeat{ix}(end-3:end))-10);
...
if 1==0
    ...
    for ix=14:size(Pfunc_social_defeat,2)    
        ndel_dummies=5;
        [fpath, fname, ext]=fileparts(Pfunc_social_defeat{ix});
        Pinput=spm_select('FPList',fpath, ['^u_del5.*' fname  '.nii']);
        cc_check_physio_dummy(Pinput,Pphysio_social_defeat(ix,:),ndel_dummies)
    end
end
...
std_filt= [3 14 9 15];
rsfilt = load('/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/02-preprocessing/physiodiagnosis/rs_filt_social_defeat.txt')
...
%         end
        [fpath, fname, ext]=fileparts(Pfunc_social_defeat{ix});
        Pinput=spm_select('FPList',fpath, ['^u_del5.*' fname  '.nii']);
        [fpath, fname, ext]=fileparts(deblank(Pphysio_social_defeat(ix,:)));
...
if 1==0
    cd([procdir filesep 'physiodiagnosis'])
    rb_print_physio_ffts_jr(Pfunc_social_defeat,Pphysio_social_defeat,'physio_overview_repaired')
end
...
if 1==0
    for ix=1:size(Pfunc_social_defeat,2);
        ...
        do_slice_time_social_defeat_jr(Pcur);
    end
end
...
if 1==0
    for ix=1:size(P3d_2,2);
        [fpath fname ext]=fileparts(Pfunc_social_defeat{ix});
        Pfunc_cur=spm_select('ExtFPList',fpath,['^u_despiked_del5_.*' fname  '.nii'],str2num(Pfunc_social_defeat{ix}(end-3:end))-5);
        Pfdm=spm_select('ExtFPList',Pdmap_1{ix},['^ZI_.*.acq0_reorient.nii'],1);
        char_all=char([Pfunc_cur;Pfunc_social_defeat(ix);P3d_2(ix);Pfdm]);
        spm_check_registration(char_all)
        input('weiter');
    end
end
...
if 1==0
    for ix=1:size(Pdmap,2);
        ...
        [fdir, fname, ext]=fileparts(Pcur);
        cd(fdir);
        Pcur1=(['a_u_despiked_del5_' fname '_c1.nii']);
        Pcur2=(['orig_a_u_despiked_del5_' fname '_c1.nii']);
        syscmd=['cp ' Pcur1 ' ' Pcur2 ];
        system(syscmd)
    end
end
...
if 1==0
    for ix=1:size(Pfunc_social_defeat,2);
        [fpath fname ext]=fileparts(Pfunc_social_defeat{ix});
        Pfunc_old=spm_select('ExtFPList',fpath,['^a1_u_despiked_del5_.*' fname  '.nii'],1);
        Pfunc_cur=spm_select('ExtFPList',fpath,['^a1_u_despiked_del5_.*' fname  '_c1.nii'],1);
        Pfunc_cur1=spm_select('ExtFPList',fpath,['^a1_u_despiked_del5_.*' fname  '_c1.nii'],str2num(Pfunc_social_defeat{ix}(end-3:end))-5);
...
if 1==0
    for ix=1:size(Pfunc_social_defeat,2);
        [fpath fname1 ext]=fileparts(Pfunc_social_defeat{ix});
...
Pdir='/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/02-preprocessing/DARTEL/';
if 1==0
    [fpath, fname, ext]=fileparts(Pfunc_social_defeat{1});
    P=spm_select('FPlist',fpath,['^st_a1_u_despiked_del5_' fname '_c1_c2t.nii']);
    V=spm_vol(P);
...
if 1==0
    ...
    for ix=1:size(P3d_2,2);
        ...
        Pfdm2=spm_select('FPList','/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/02-preprocessing/DARTEL/',['^rc1bc_st_.*_c2t.nii']);
...
if 1==0
    for ix=1:length(Pfunc_social_defeat)
        ...
        Pref=spm_select('ExtFPlist','/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/02-preprocessing/DARTEL/',['^rc1bc_st_' fname '_c2t.nii'],1);
...
if 1==0
    clear Pcur1 Pcur2
    Pwdir='/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/02-preprocessing/DARTEL/';
...
if 1==0
    for ix = 1:length(P3d_2);
        clear job
        [fpath, fname, ext]=fileparts(P3d_2{ix});
        job.template = {'/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/02-preprocessing/DARTEL/Template_6.nii'};
        job.data.subj.flowfield = { spm_select('FPlist','/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/02-preprocessing/DARTEL/',['^u_rc1bc_st_' fname '_c2t_Template.nii'])};
...
if 1==0
    addpath(genpath('/home/jonathan.reinwald/Programs/spm12'))
    for ix=1:size(Pfunc_social_defeat,2)
        Pmask='/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/02-preprocessing/DARTEL/mask_template_6_polished.nii';
        %% Social
        [fpath, fname, ext]=fileparts(Pfunc_social_defeat{ix});
        Pfunc_cur=spm_select('FPlist',fpath,['^wrst_a1_u_despiked_despiked_del5_' fname '_c1_c2t.nii$']);
        [fpath, fname, ext]=fileparts(P3d_2{ix});
...
if 1==0
    for ix=1:length(Pfunc_social_defeat);
        [fpath, fname, ext]=fileparts(Pfunc_social_defeat{ix});
        Pfunc_cur=spm_select('FPlist',fpath,['^wst_a1_u_despiked_del5_' fname '_c1_c2t.nii$']);
...
if 1==0
    for ix=1:size(Pfunc_social_defeat,2)
        %% Social Defeat
        [fpath, fname, ext]=fileparts(Pfunc_social_defeat{ix});
        rp=spm_load(spm_select('FPlist',fpath,'^rp_despiked_del5.*.txt'));
        rp_diff=[zeros(1,size(rp,2)); diff(rp)];
        rp_diff_minus2=[rp_diff([3:(str2num(Pfunc_social_defeat{ix}(end-3:end))-5)],:); zeros(2,size(rp_diff,2))];
        rp_diff_minus1=[rp_diff([2:(str2num(Pfunc_social_defeat{ix}(end-3:end))-5)],:); zeros(1,size(rp_diff,2))];
        rp_diff_plus1=[zeros(1,size(rp_diff,2));rp_diff([1:(str2num(Pfunc_social_defeat{ix}(end-3:end))-6)],:)];
...
if 1==0
    P_template='/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/02-preprocessing/DARTEL/Template_6.nii';
...
if 1==0
    ...
    for ix=1:size(Pfunc_social_defeat,2);
        %% social_defeat
        [fpath, fname, ext]=fileparts(Pfunc_social_defeat{ix});
        EPI=spm_select('ExtFPlist',fpath,['^wrst_a1_u_despiked_del5_' fname '_c1_c2t.nii']);
        thres=0.10;
        mask='/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/02-preprocessing/DARTEL/mask_template_6_polished.nii';
        outputdir=motiondir;
...
if 1==0
    for ix=1:size(Pfunc_social_defeat,2)
        %% social_defeat
        [fpath fname ext]=fileparts(Pfunc_social_defeat{ix});
        Pcur=spm_select('ExtFPList',fpath,['^regfilt_motcsfder_wrst_a1_u_del5_' fname '_c1_c2t.nii$'],1);
        fwhm_cur=[6 6 6];
...
if 1==0
    ...
    for ix=1:size(P3d,2);
        ...
        Pfunc_norm=spm_select('ExtFPList',fpath,['^wst_a_u_del5_' fname '_c1_c2t.nii'],1);
        
        Pfunc_smooth=spm_select('ExtFPList',fpath,['^s_wst_a_u_del5_' fname '_c1_c2t.nii'],1);
...
if 1==0
    for ix=1:size(Pfunc_social_defeat,2)
        %% social_defeat
        [fpath fname ext]=fileparts(Pfunc_social_defeat{ix});
        Pcur=spm_select('ExtFPList',fpath,['^wrst_a1_u_del5_' fname '_c1_c2t.nii$'],1);
        fwhm_cur=[6 6 6];
        do_smooth_lw(Pcur,fwhm_cur);
    end
end
...
if 1==0
    for ix=1:size(Pfunc_social_defeat,2);
        ...
        Pcur=deblank(Pfunc_social_defeat{ix});
        [fpath, fname, ext]=fileparts(Pcur);
        ICA_dir = '/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/02-social_defeat/02-preprocessing/ICA';
        mkdir(ICA_dir);
