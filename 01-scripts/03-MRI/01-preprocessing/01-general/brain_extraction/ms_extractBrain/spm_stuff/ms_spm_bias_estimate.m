function T = ms_spm_bias_estimate(V,flags)
...
def_flags = struct('nbins',1024,'reg',0.001,'cutoff',80);
if nargin < 2,
        flags = def_flags;
else
...
for iter = 1:128,
    % this mex function is an issue.. it wants to have a V
...
%         corImg(:,:,p) = single(img);
%     end
%     figure(21); imagesc(squeeze(corImg(:,90,:)))
    if iter==1,
        close(figure(11));
        fg = figure(11);
        if ~isempty(fg),
...
%     if olpp-lpp < 1e-5, break; end;
    if olpp-lpp < 1e-3, break; end;
    olpp = lpp;
end;
return;
%=======================================================================
