...
% Jan21: ms_do_brainExtraction is the new, clean version of ms_PCNN3D_v2
...
if nargin <2
    fprintf('No estimated brain size given! ');
    if totVol < 10000 % that should work; maybe better to calculate the whole volume size
        BrSize=[350,650]*1000; % brain size range for mouse (mm3). If you use 10x data, this should be 10x too (for each dimension!)
        fprintf('I guess we deal with mouse data.\n');
    else
        BrSize=[1200,3000]*1000; % brain size range for RAT (mm3)
        fprintf('I guess we deal with rat data.\n');
    end
end
