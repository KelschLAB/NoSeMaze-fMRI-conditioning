function acl_regfilt_motcsf_awake_despiked_jr(EPI,csf,GM_mask,Pmask,qmcsf,a)
...
rp_only=[spm_load(spm_select('FPList',fdir,'^rp_despiked.*.reorient.txt'))];
rp_only_diff=[rp_only [zeros(1,size(rp_only,2)); diff(rp_only)]];

rp0=[spm_load(spm_select('FPList',fdir,'^rp_despiked.*.reorient.txt')) nanmean(datanorm(find(csf),:))'];
rp1=[rp0 [zeros(1,size(rp0,2)); diff(rp0)]];



% not detrended rp:
dlmwrite(fullfile(fdir,strcat('regressors_despiked_mot_der.txt')),rp_only_diff,'delimiter','\t','precision','%.6f')

dlmwrite(fullfile(fdir,strcat('regressors_despiked_motcsf.txt')),rp0,'delimiter','\t','precision','%.6f')
dlmwrite(fullfile(fdir,strcat('regressors_despiked_motcsf_der.txt')),rp1,'delimiter','\t','precision','%.6f')

%--------------------------------------------------------------------------
% create versions with detrendet data
% detrending
% 
rp=spm_load(spm_select('FPList',fdir,'^rp_despiked.*.reorient.txt')) ;
...
dlmwrite(fullfile(fdir,strcat('regressors_despiked_detrmot_csf.txt')),rp2,'delimiter','\t','precision','%.6f')
dlmwrite(fullfile(fdir,strcat('regressors_despiked_detrmot_csfgs.txt')),rp3,'delimiter','\t','precision','%.6f')
dlmwrite(fullfile(fdir,strcat('regressors_despiked_detrmot_csfgs_der.txt')),rp4,'delimiter','\t','precision','%.6f')
dlmwrite(fullfile(fdir,strcat('regressors_despiked_detrmot_csf_der.txt')),rp5,'delimiter','\t','precision','%.6f')


%% Regression:

if 1==a
    rpfile=fullfile(fdir,strcat('regressors_despiked_motcsf_der.txt'));
