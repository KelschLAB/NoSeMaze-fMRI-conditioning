function coolfieldmap=fieldmap_fill_dilate_jr(Pfm)
...
spm2_fm=Pfm%spm_select();% 'ExtFPlist',Pfmdir,'^demedianed_fpm_Z.*_c1_spm2.img')
V=spm_vol(spm2_fm);
imgmat=spm_read_vols(V);
imgmat_enl=double(imgmat~=0);
img_lrg=imgmat_enl;
for jx=1:5;
    img_lrg = spm_dilate(img_lrg);
end;
img_lrg = double(img_lrg>0);
...
ind2=find(img_lrg);              %indices of nonzero elements
maskmat2=zeros(size(img_lrg),'int8');
maskmat2(ind2)=1;                %mask of existing field map
...
indz=find(maskmat==0 & maskmat2==1);      %vector of indices of zeros    
...
newmap=fullfile(vpath, ['full_', vname, ext]);
