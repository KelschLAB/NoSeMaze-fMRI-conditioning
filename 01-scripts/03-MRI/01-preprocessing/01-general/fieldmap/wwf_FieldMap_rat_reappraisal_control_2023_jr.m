...
function res=wwf_FieldMap_rat_reappraisal_control_2023_jr(Pdir)
start=pwd;
if nargin < 1
...
for i=1:size(Pdir,1)
    ...
    %cd(start)
    %cd(deblank(Pdir(i,:)))
    fieldmap_a=spm_select('FPlist',Pcur,['^Z.*_p1_reorient.nii$']) %%% acq0_reorient.nii is the FieldMap" 
    fieldmap_b=spm_select('FPlist',Pcur,['^z.*_p1_reorient.nii$'])
    fieldmap=[fieldmap_a;fieldmap_b];
...
%     val=str2double(line(1:loc(1)))
    brkhdr_a=spm_select('FPlist',Pdir,['^Z.*.brkhdr']); 
    brkhdr_b=spm_select('FPlist',Pdir,['^z.*.brkhdr']);%% defining the path of the header file of the FieldMap as brkhdr
    brkhdr = [brkhdr_a;brkhdr_b];
