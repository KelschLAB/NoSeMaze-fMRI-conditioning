...
% combined or separated hemisphere
...
if separated_hemisphere==1
    main_dir = '/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/01-reappraisal/12-gPPI/separated_hemisphere';
elseif separated_hemisphere==0
    main_dir = '/home/jonathan.reinwald/ICON_Autonomouse/03-processed-data/03-MRI/01-reappraisal/12-gPPI/combined_hemisphere';
end
if exist(main_dir)~=7
    mkdir(main_dir);
end
...
% save_dir = fullfile(work_dir,'plots_symmetric');
...
if 1==0
    % Loop over animals
    for subj_ix = 1:length(Pfunc_reappraisal)
        
        % Select functional data
        [fdir, fname, ext]=fileparts(Pfunc_reappraisal{subj_ix});
...
% load(fullfile(work_dir,'results_PPI.mat'),'PPI_betas_all');
...
if 1==0
    ...
    [fdir, fname, ext]=fileparts(Pfunc_reappraisal{1});
    subjAbrev = fname(1:6);
    spm_file=fullfile(GLM_dir,'firstlevel',subjAbrev,'SPM.mat');
    load(spm_file);
    ...
    % choose your PPI
    % PPI_betas_all_cur = PPI_betas_all;
    for block_ix=1:size(PPI_betas_all,3)
        for subj_ix=1:size(PPI_betas_all,4)
            PPI_betas_all(:,:,block_ix,subj_ix) = (squeeze(PPI_betas_all(:,:,block_ix,subj_ix))+squeeze(PPI_betas_all(:,:,block_ix,subj_ix))')./2;
        end
    end
    ...
    save(fullfile(work_dir,'results_PPI_symmetric.mat'),'res','res_comp','PPI_betas_all');
end
...
%     load(fullfile(work_dir,'results_PPI_symmetric.mat'),'res','res_comp');
    ...
    for res_ix=2
        ...
        for cond_ix=42%:length(res)
            % plots
            fig(cond_ix)=figure('visible', 'on');
