function do_seedanalysis_secondlevel_jr(P_sc1, P_sc2, P_outputdir, Pmsk_general, contrast)

if exist(P_outputdir)~=7
    mkdir(P_outputdir);
    
    % Factorial Design Specification
    matlabbatch{1}.spm.stats.factorial_design.dir = cellstr(P_outputdir);
                        
    for sx = 1:size(P_sc1,1)
        matlabbatch{1}.spm.stats.factorial_design.des.pt.pair(sx).scans = {P_sc1(sx,:);P_sc2(sx,:)};
    end
    matlabbatch{1}.spm.stats.factorial_design.des.pt.gmsca = 0; % grand mean scaling (only PET/VBM);
    matlabbatch{1}.spm.stats.factorial_design.des.pt.ancova = 0; % ANCOVA (only for PET/VBM)
