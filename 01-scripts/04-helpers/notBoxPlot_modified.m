...
for ii=1:length(uX)
    f=b==ii;
    [hTemp,statsTemp,myX(ii,:),myY(ii,:)]=myPlotter(x(f),y(:,f));
    H = [H,hTemp];
...
if size(myX,1)==2
    ...
    hold on;
    scatter(myX(1,:),myY(1,:),'MarkerEdgeColor','none','MarkerFaceColor',[0.45 0.45 .8],'SizeData',0.1);
    hold on;
    scatter(myX(2,:),myY(2,:),'MarkerEdgeColor','none','MarkerFaceColor',[0.45 0.45 .8],'SizeData',0.1);

    for ix=1:size(myX,2)
        if myY(2,ix) > myY(1,ix)
            line(myX(:,ix),myY(:,ix),'Color',[.6 .6 .6],'LineWidth',1.2,'LineStyle','--');
        elseif myY(2,ix) < myY(1,ix)
            line(myX(:,ix),myY(:,ix),'Color',[.6 .6 .6],'LineWidth',1.2,'LineStyle','--');
        end
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [h,statsOut,thisX,thisY]=myPlotter(X,Y)
    ...
    for k=1:length(X)
        ...
        if strcmp(style,'patch') || strcmp(style,'sdline')
            h(k).semPtch=patchMaker(SEM(k),[0.6,0.6,0.6]);
            h(k).mu=plot([X(k)-jitScale,X(k)+jitScale],[mu(k),mu(k)],'color',[0 0 0],...
                'linewidth',2);
            if markMedian
    ...
    if strcmp(style,'line')
        for k=1:length(X)
            ...
            h(k).mu=plot(X(k),mu(k),'o','color',[0 0 0],...
            ...
            h(k).sem=plot([X(k),X(k)],[mu(k)-SEM(k),mu(k)+SEM(k)],'-r',...
                'linewidth',2);   
            if markMedian
                h(k).med=plot(X(k),med(k),'s','color',[0,0,0],...
