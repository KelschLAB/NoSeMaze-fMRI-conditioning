function varargout=notBoxPlot_modified_pupilANDlid(y,x,varargin)
...
%% ADDED BY JR
for ix=1:size(myX,1)
    scatter(myX(ix,:),myY(ix,:),'MarkerEdgeColor','none','MarkerFaceColor','none');
end

if size(myY,1)>2
    for ix=1:size(myX,2)
        if myY(3,ix) > myY(1,ix)
            line(myX(:,ix),myY(:,ix),'Color',[.8 .8 .8],'LineWidth',1,'LineStyle','--');
        elseif myY(3,ix) < myY(1,ix)
            line(myX(:,ix),myY(:,ix),'Color',[.8 .8 .8],'LineWidth',1,'LineStyle','--');
        end
    end
end
